...
import { selectAddress } from '../../features/productSlice';
import AddressModal from './AddressModal';

function Cart() {
    const dispatch = useDispatch();
    const { cart, loading, address } = useSelector((state) => state.app);
    ...
    const handleCheckout = () => {
        ...
        setIsModalOpen(true); // Open the modal when proceeding to checkout
    }

    const handleCloseModal = () => {
        setIsModalOpen(false); // Close the modal when it's dismissed
    }
                ...
                <div className="flex justify-between mx-4 ">
                    ...
                    <div className="flex flex-wrap flex-1 mt-6">
                        {cart && cart.map((ele, index) => (
                            // <!-- component -->
                            <div key={index} className="bg-white border  border-gray-200 p-2 w-80 max-w-3xl sm:w-full sm:p-4 h-45 shadow-lg flex flex-col sm:flex-row gap-5 select-none">
                                <div
